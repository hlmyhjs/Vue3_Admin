<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" height="455" border :data="typeList" @selection-change="handleSelectionChange">
...
function handleUpdate(row) {
  reset()
  const dictId = row.dict_id || ids.value
...
function submitForm() {
  proxy.$refs['dictRef'].validate((valid) => {
    if (valid) {
      if (form.value.dict_id != undefined) {
...
function handleDelete(row) {
  const dictIds = row.dict_id || ids.value
  proxy.$modal
