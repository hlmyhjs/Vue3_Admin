...
<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" height="455" :data="dataList" border @selection-change="handleSelectionChange">
      ...
      <el-form ref="dataRef" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="回显样式" prop="list_class">
          <el-select v-model="form.list_class">
            <el-option
              v-for="item in list_classOptions"
              :key="item.value"
...
const total = ref(0)
const title = ref('')
const defaultdict_type = ref('')
const typeOptions = ref([])
const route = useRoute()
// 数据标签回显样式
const list_classOptions = ref([
...
const data = reactive({
  ...
  rules: {
    dict_label: [{ required: true, message: '数据标签不能为空', trigger: 'blur' }],
    dict_value: [{ required: true, message: '数据键值不能为空', trigger: 'blur' }],
...
/** 查询字典类型详细 */
function getTypes(dict_id) {
  getType(dict_id).then((response) => {
    queryParams.value.dict_type = response.data.dict_type
    defaultdict_type.value = response.data.dict_type
    getList()
  })
}
...
function resetQuery() {
  ...
  queryParams.value.dict_type = defaultdict_type
  handleQuery()
}
/** 新增按钮操作 */
function handleAdd() {
...
function handleSelectionChange(selection) {
  ids.value = selection.map((item) => item.dict_code)
...
function handleUpdate(row) {
  reset()
  const dict_code = row.dict_code || ids.value
...
function submitForm() {
  proxy.$refs['dataRef'].validate((valid) => {
    if (valid) {
      if (form.value.dict_code != undefined) {
        updateData(form.value).then((response) => {
          useDictStore().removeDict(queryParams.value.dict_type)
...
function handleDelete(row) {
  const dict_codes = row.dict_code || ids.value
  proxy.$modal
    .confirm('是否确认删除字典编码为"' + dict_codes + '"的数据项？')
    .then(function () {
